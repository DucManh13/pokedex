...
function PkInfo() {
  ...
  useEffect(() => {
    let mounted = true;
    axios
      .get('https://pokeapi.co/api/v2/pokemon/' + pkId)
      .then((response) => {
    ...
    axios
      .get('https://pokeapi.co/api/v2/pokemon-species/' + pkId)
      .then((response) => {
  ...
  return !(data && species) ? null : (
    <div className="container bg-dark text-white font-weight-normal p-3">
          ...
          <tr>
            <td className="align-middle text-center">
              {species.genera.map((item) =>
                item.language.name === 'en' ? item.genus : null,
              )}
            </td>
            <td className="align-middle text-center">
              {species.names.map((name, index) =>
                name.language.name === 'roomaji' ||
            ...
            <td className="align-middle text-center">{data.height / 10} m</td>
            <td className="align-middle text-center">{data.weight / 10} kg</td>
          </tr>
        </tbody>
      </table>
      ...
      <table className="table table-dark text-center table-bordered mb-0">
        ...
        <tbody>
          <tr>
            ...
            <td className="align-middle text-center">
              {species.egg_groups
                .map((group) => formatText(group.name))
                .join(', ')}
            </td>
          </tr>
        </tbody>
      ...
      <table className="table table-dark text-center table-bordered mb-0">
        ...
        <tbody>
          <tr>
            <td className="align-middle text-center">{data.base_experience}</td>
            <td className="align-middle text-center">{species.capture_rate}</td>
            <td className="align-middle text-center">
      ...
      <div className="container font-weight-bold px-0">
        ...
        {data.stats.map((stat, index) => (
          <span key={index} className="d-flex">
            <div
              className={`progress-bar mt-1 ${stat.stat.name}`}
      ...
      <table className="table table-dark table-bordered text-center">
        ...
        <tbody>
          <tr>
            <td className="align-middle">Default</td>
            <td>
              <img
      ...
      {data.sprites?.other?.dream_world?.front_default && (
        <img
          ...
          className="img-fluid"
          alt="Pokemon Dream World Sprite"
        />
      )}
    </div>
  );
