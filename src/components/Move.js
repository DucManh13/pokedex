...
function Ability(props) {
  ...
  useEffect(()=>{
    let mounted=true;
    axios.get('https://pokeapi.co/api/v2/move/'+moveId) 
        .then(response => {
  ...
  return !data?null:(
    <div className="container-fluid">
      ...
      <h5 className="pb-4 font-weight-normal">
          Description: {data.effect_entries[0].effect.replace('$effect_chance',data.effect_chance)}<br/>
          Power: {data.power|0}<br/>
          PP: {data.pp}<br/>
          Accuracy: {data.accuracy}<br/>
          Priority: {data.priority}<br/>
          Category: {formatText(data.damage_class.name)}<br/>
      ...
      <table className="table table-hover table-dark text-center">
        ...
        <tbody>
          {data.learned_by_pokemon.map((item,index)=><Pkm key={index} url={item.url}/>)}
        </tbody>
      </table>    
    </div>
  );
